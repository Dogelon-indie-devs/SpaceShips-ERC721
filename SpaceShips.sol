...
contract DogelonSpaceShipNFT is ERC1155, Ownable {
    ...
    mapping (uint256 => string) BluePrintURIS;
    ...
    function uri(uint256 _TokenID) override public view returns (string memory) {    
      ...
      if (MintedTokens[_TokenID]) {
        ...
        MainURI = BluePrintURIS[ExtractGenerationIDByTokenID(_TokenID)];   
      }     
      return(MainURI);             
    }
 
    function SetBluePrintURI(uint256 _GenerationID, string memory _NewURI) public onlyOwner{
      BluePrintURIS[_GenerationID] = _NewURI; 
    }
